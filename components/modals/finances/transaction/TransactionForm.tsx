...
// Make sure this type matches the actual data you pass in!
...
export const TransactionForm: React.FC<AddTransactionModalProps> = ({
  ...
  // Maps "INCOME" or "IN" to the INCOME tab, otherwise defaults to EXPENSE
  const [transactionType, setTransactionType] = useState<"INCOME" | "EXPENSE">(
    initialData?.type === "INCOME" || initialData?.type === "IN" ? "INCOME" : "EXPENSE"
  );

  // Extracts the ID whether it's stored as a string, a populated object, or the 'item' from your JSON
  ...
  // Helper to format the Date string for the <input type="date"> field (requires YYYY-MM-DD)
      ...
      {initialData?._id && <input type="hidden" name="id" value={initialData._id} />}
        ...
        <div className="flex bg-shaded p-1 rounded-xl border border-border shrink-0">
          <button
            ...
            className={`flex-1 py-2 text-xs font-black rounded-lg transition-all duration-300 ${transactionType === "EXPENSE"
          ...
          <button
            ...
            className={`flex-1 py-2 text-xs font-black rounded-lg transition-all duration-300 ${transactionType === "INCOME"
        ...
        <div className="grid bg-shaded rounded-2xl p-6 grid-cols-1 md:grid-cols-2 gap-6 border border-border shrink-0">
          <FormField
            ...
            // 2. ADD DEFAULT VALUES (falls back to quantity if using inventory JSON)
        ...
        <FormField
          ...
          // Falls back to "reason" if using the inventory JSON
