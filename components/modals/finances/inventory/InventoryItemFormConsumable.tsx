    ...
    return (
        <form action={formAction} className="flex flex-col h-full w-full animate-in fade-in duration-500">
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-8 space-y-8 custom-scrollbar">
                {/* LOGISTICS HEADER */}
                <div className="flex items-center gap-4 bg-primary/5 p-6 rounded-[2.5rem] border border-primary/20 mt-2 shadow-sm">
                    <div className="w-14 h-14 bg-primary text-text-invert rounded-2xl flex items-center justify-center shadow-glow-primary">
                        <Box size={28} />
                    </div>
                    <div>
                        <h2 className="text-[10px] font-black text-primary uppercase tracking-[0.4em]">Logistics Registry</h2>
                        <p className="text-sm font-black text-text">Consumable Goods & Supplies</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <div className="lg:col-span-7 bg-card p-8 rounded-[2rem] border border-border space-y-6 shadow-sm">
                        ...
                        <FormField label="Item Name *" name="name" required placeholder="e.g. Sona Mansuli Rice" defaultValue={item?.name} />
                    ...
                    <div className="lg:col-span-5 bg-shaded p-8 rounded-[2rem] border-2 border-dashed border-primary/20 space-y-6">
                        ...
                        <FormField label="Unit (kg/pkt/ltr)" name="unit" required defaultValue={item?.unit || "kg"} />
                        <FormField label="Low Stock Alert" name="minimumStockLevel" type="number" defaultValue={item?.minimumStockLevel} placeholder="Notify when below..." />
                    </div>
                </div>

                <div className="bg-card p-6 rounded-3xl border border-border">
                    <FormField label="Brand / Remarks" name="description" placeholder="Specific brand or quality notes..." defaultValue={item?.description} />
                </div>
            </div>

            <div className="shrink-0 flex justify-end gap-5 p-6 border-t border-border bg-card/80 backdrop-blur-md rounded-t-[2.5rem]">
                <button type="button" onClick={closeModal} className="text-[10px] font-black uppercase text-text-muted hover:text-text tracking-widest">Discard</button>
                <Button type="submit" disabled={isPending} className="px-14 h-14 font-black text-xs uppercase tracking-[0.2em] text-text-invert bg-primary shadow-glow-primary transition-all active:scale-95">
